...
public class SponsorsController implements Initializable {
    ...
    private void setupColumns() {
        ...
        colTelephone.setCellValueFactory(cellData ->
                new javafx.beans.property.SimpleStringProperty(
                        cellData.getValue().getTel() != null ? cellData.getValue().getTel() : ""));

        colAdresse.setCellValueFactory(cellData ->
                new javafx.beans.property.SimpleStringProperty(
                        cellData.getValue().getAdresse() != null ? cellData.getValue().getAdresse() : ""));
    ...
    private void loadSponsors() {
        try {
            ...
            updateStats(sponsorsList); // ✅ important
    ...
    private void handleSearch() {
        try {
            ...
            ObservableList<Sponsor> results =
                    FXCollections.observableArrayList(sponsorService.rechercher(keyword));

            sponsorTable.setItems(results);
            updateStats(results); // ✅ important

        } catch (SQLException e) {
            showAlert("Erreur", "❌ Erreur recherche: " + e.getMessage());
        }
    }
    ...
    private void handleSort() {
        try {
            ...
            showAlert("Erreur", "❌ Erreur tri: " + e.getMessage());
        }
    }
    ...
    private void addSponsor(ActionEvent event) {
        ...
        if (dialog.isApproved()) {
            loadSponsors();
            showAlert("Succès", "✅ Sponsor ajouté !");
        }
    }
    ...
    private void editSponsor(ActionEvent event) {
        ...
        if (selected == null) {
            showAlert("Attention", "⚠️ Sélectionnez un sponsor");
            return;
        }

        try {
            Sponsor full = sponsorService.findById(selected.getId());
            SponsorFormDialog dialog = new SponsorFormDialog(full);
            dialog.show();
